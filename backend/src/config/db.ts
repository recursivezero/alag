...
const existingUserSessionColumns = new Set(userSessionColumns.map((row: { COLUMN_NAME: string }) => row.COLUMN_NAME));
...
if (!existingColumns.has("mobile_verified")) {
  await db.execute("ALTER TABLE users ADD COLUMN mobile_verified BOOLEAN NOT NULL DEFAULT FALSE AFTER is_verified");
}
...
const [otpColumns]: any = await db.execute(`
  SELECT COLUMN_NAME
  FROM INFORMATION_SCHEMA.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'otp_codes'
`);
const existingOtpColumns = new Set(otpColumns.map((row: { COLUMN_NAME: string }) => row.COLUMN_NAME));

if (!existingOtpColumns.has("identifier")) {
  await db.execute("ALTER TABLE otp_codes ADD COLUMN identifier VARCHAR(255) NULL AFTER email");
  await db.execute("UPDATE otp_codes SET identifier = email WHERE identifier IS NULL");
}
if (!existingOtpColumns.has("type")) {
  await db.execute("ALTER TABLE otp_codes ADD COLUMN type VARCHAR(10) NOT NULL DEFAULT 'email' AFTER identifier");
}
if (!existingOtpColumns.has("verified")) {
  await db.execute("ALTER TABLE otp_codes ADD COLUMN verified BOOLEAN NOT NULL DEFAULT FALSE AFTER otp");
}
...
const [pendingColumns]: any = await db.execute(`
  SELECT COLUMN_NAME
  FROM INFORMATION_SCHEMA.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'pending_registrations'
`);
const existingPendingColumns = new Set(pendingColumns.map((row: { COLUMN_NAME: string }) => row.COLUMN_NAME));

if (!existingPendingColumns.has("mobile")) {
  await db.execute("ALTER TABLE pending_registrations ADD COLUMN mobile VARCHAR(30) NULL AFTER password");
}
if (!existingPendingColumns.has("email_verified")) {
  await db.execute("ALTER TABLE pending_registrations ADD COLUMN email_verified BOOLEAN NOT NULL DEFAULT FALSE AFTER mobile");
}
if (!existingPendingColumns.has("mobile_verified")) {
  await db.execute("ALTER TABLE pending_registrations ADD COLUMN mobile_verified BOOLEAN NOT NULL DEFAULT FALSE AFTER email_verified");
}
...
const existingAdminColumns = new Set(adminColumns.map((row: { COLUMN_NAME: string }) => row.COLUMN_NAME));
...
const existingPostColumns = new Set(postColumns.map((row: { COLUMN_NAME: string }) => row.COLUMN_NAME));
...
async function seedDatabase(): Promise<void> {
  const [anyAdminRows]: any = await db.execute("SELECT id FROM admins LIMIT 1");

  if (!anyAdminRows.length) {
    const defaultAdminEmail = process.env.DEFAULT_ADMIN_EMAIL || "admin@gmail.com";
    const defaultAdminPassword = await hashPassword(process.env.DEFAULT_ADMIN_PASSWORD || "Admin@1234");

    await db.execute("INSERT INTO admins (full_name, email, password, role) VALUES (?, ?, ?, ?)", [
      "Admin User",
      defaultAdminEmail,
    ...
    console.log(`[seed] Created default admin "${defaultAdminEmail}".`);

    if (!process.env.DEFAULT_ADMIN_PASSWORD) {
      console.warn(
        `[seed] Default admin "${defaultAdminEmail}" was seeded with the built-in default password. ` +
          "Log in and change it immediately, or set DEFAULT_ADMIN_EMAIL/DEFAULT_ADMIN_PASSWORD before first boot.",
      );
    }
  }
  ...
  if (!existingPosts.length) {
    const [seedUsers]: any = await db.execute("SELECT id, COALESCE(full_name, name) AS name FROM users ORDER BY id ASC LIMIT 3");
    ...
    if (seedUsers.length) {
      ...
      if (firstPost) {
        ...
        await db.execute("INSERT INTO post_comments (post_id, user_id, body) VALUES (?, ?, ?)", [
          firstPost.id,
          primaryUserId,
          "The light on this one is unreal.",
        ]);
        await db.execute("INSERT INTO post_comments (post_id, user_id, body) VALUES (?, ?, ?)", [
          firstPost.id,
          secondaryUserId,
          "Bookmarking this for my next beach trip.",
        ]);
      }
      ...
      if (secondPost) {
        ...
        await db.execute("INSERT INTO post_comments (post_id, user_id, body) VALUES (?, ?, ?)", [
          secondPost.id,
          tertiaryUserId,
          "Those ridgelines are incredible.",
        ]);
      }

      console.log(`[seed] Created ${seedPosts.length} demo posts.`);
    }
  }
  ...
  const [deletedMetricRows]: any = await db.execute("SELECT metric_key FROM admin_dashboard_metrics WHERE metric_key = ? LIMIT 1", [
    "deleted_users_total",
  ]);

  if (!deletedMetricRows.length) {
    const [deletedCountRows]: any = await db.execute("SELECT COUNT(*) AS total FROM user_activity_logs WHERE event_type = 'deleted'");

    await db.execute("INSERT INTO admin_dashboard_metrics (metric_key, metric_value) VALUES (?, ?)", [
      "deleted_users_total",
      Number(deletedCountRows[0]?.total || 0),
    ]);
  }

  const [activityRows]: any = await db.execute("SELECT DISTINCT user_id AS userId FROM user_activity_logs");

  const loggedUserIds = new Set(activityRows.map((row: { userId: number }) => Number(row.userId)));

  if (!loggedUserIds.size) {
    const [users]: any = await db.execute("SELECT id, created_at AS createdAt, is_disabled AS isDisabled FROM users ORDER BY id ASC");

    for (const user of users as Array<{ id: number; createdAt: string; isDisabled: number }>) {
      await db.execute("INSERT INTO user_activity_logs (user_id, event_type, created_at) VALUES (?, ?, ?)", [
        user.id,
        "created",
        user.createdAt,
      ]);
      ...
      if (Number(user.isDisabled)) {
        await db.execute("INSERT INTO user_activity_logs (user_id, event_type, created_at) VALUES (?, ?, CURRENT_TIMESTAMP)", [
          user.id,
          "disabled",
        ]);
      }
