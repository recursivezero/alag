...
const [existingAdminRows]: any = await db.execute("SELECT id FROM admins WHERE email = ? LIMIT 1", ["admin@gmail.com"]);
...
if (!existingAdminRows.length) {
  ...
  await db.execute("UPDATE admins SET full_name = ?, password = ?, role = ? WHERE email = ?", [
    "Admin User",
    defaultAdminPassword,
    "admin",
    "admin@gmail.com",
